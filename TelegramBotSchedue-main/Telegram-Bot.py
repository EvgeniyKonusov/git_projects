...
def setup(message):
    ...
    if message.chat.id == 870779877 or message.chat.id == 1160704466:
        ...
        bot.send_message(message.chat.id, "Валерия, выбери день, а потом время в которые ты бы хотела проводить занятия, остальные дни для пользовотелей доступны не будут! 😌", reply_markup=markup)   
    else:
        get_all_bd()
...
def get_time(message):
    ...
    if message.text == "Понедельник":  #  2
        ...
        bot.send_message(message.chat.id, f"И тебе привет, <b>{message.from_user.first_name}</b>, предлагаю записаться на урок! Проверь актуальное расписание! ;)", parse_mode='html')
